...
namespace TheEngine::ECS
{
	...
	bool ArchetypeManager::moveArchetypeChunkHeaderToAvailableList(ArchetypeChunkHeader* archetypeChunkHeader)
	{
		...
		if (archetypeChunkHeader == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeChunkHeader is nullptr in moveArchetypeChunkHeaderToAvailableList");
		...
		if (archetypeDefinition == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeDefinition is nullptr in moveArchetypeChunkHeaderToAvailableList");
		...
		if (it != m_archetypeChunksMap.end())
		{
			...
			if (!moveResult)
			{
				...
				m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to move ArchetypeChunkHeader To AvailableList");
		...
		else
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeSignature not found in moveArchetypeChunkHeaderToAvailableList");
	...
	bool ArchetypeManager::moveArchetypeChunkHeaderToFullList(ArchetypeChunkHeader* archetypeChunkHeader)
	{
		...
		if (archetypeChunkHeader == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeChunkHeader is nullptr in moveArchetypeChunkHeaderToFullList");
		...
		if (archetypeDefinition == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeDefinition is nullptr in moveArchetypeChunkHeaderToFullList");
		...
		if (it != m_archetypeChunksMap.end())
		{
			...
			if (!moveResult)
			{
				...
				m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to move ArchetypeChunkHeader To FullList");
	...
	ArchetypeManager::MoveResult ArchetypeManager::moveEntityData(ArchetypeChunkHeader* destArchetypeChunkHeader, ArchetypeChunkHeader* srcArchetypeChunkHeader, EntityRecord* entityToBeMovedRecord)
	{
		...
		if (srcArchetypeChunkHeader == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Source ArchetypeChunkHeader is nullptr in moveEntityData");
		...
		if (destArchetypeChunkHeader == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Destination ArchetypeChunkHeader is nullptr in moveEntityData");
		...
		if (entityToBeMovedRecord == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : EntityRecord is nullptr in moveEntityData");
		...
		if (srcArchetypeChunkHeader->chunkEntityUsed == 0)//means there is no entity in source 
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Corruption in srcArchetypeChunkHeader detected,chunkEntityUsed in srcArchetypeChunkHeader cannot be zero");
		...
		if (destArchetypeChunkHeader->chunkEntityUsed == MAX_NUM_OF_ENTITIES_PER_CHUNK)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to move destArchetypeChunkHeader to FullList");
		...
		if (srcArchetypeChunkHeader->chunkEntityUsed == MAX_NUM_OF_ENTITIES_PER_CHUNK - 1)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to move srcArchetypeChunkHeader to AvaialbleList");
	...
	std::vector<EntityRecordUpdate> ArchetypeManager::transferAndAddEntityDataToArchetypeChunk(ArchetypeChunkHeader* destArchetypeChunkHeader, ArchetypeChunkHeader* srcArchetypeChunkHeader, EntityAddInfo entityAddInfo)
	{
		...
		if (!moveResult.moveResult)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to transfer entityData to destinationArchetypeChunk");
	...
	EntityRecordUpdate ArchetypeManager::addInitialComponentDataToArchetypeChunk(ArchetypeChunkHeader* archetypeChunkHeader, EntityAddInfo entityAddInfo)
	{
		...
		if (archetypeChunkHeader == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeChunkHeader is null in addComponentDataToArchetypeChunk");
		...
		if (archetypeChunk == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeChunk is nullptr in addComponentDataToArchetypeChunk");
		...
		if (archetypeRecordChunk == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeRecordChunk is nullptr in addComponentDataToArchetypeChunk");
		...
		if (archetypeDefinition == nullptr)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeDefinition is nullptr in addComponentDataToArchetypeChunk");
			...
			for (const ComponentLayout& destComponentLayout : destComponentLayouts)
			{
				if (srcComponentData.componentId == destComponentLayout.componentId)
				{
					...
					if ((archetypeChunkEndAddress) < (destComponentAddress + destComponentLayout.componentTypeInfo->size)) // safety check
					{
						...
						m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ArchetypeChunk OVERFLOW detected during component data addition. ");
		...
		if (archetypeChunkHeader->chunkEntityUsed == MAX_NUM_OF_ENTITIES_PER_CHUNK)
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to move ArchetypeChunkHeader to fullList in ChunkList ");
	...
	ArchetypeDefinition* ArchetypeManager::createNewArchetypeDefinition(const ArchetypeSignature& archetypeSignature)
	{
		...
		for (ComponentId componentId = 0; componentId < MAX_COMPONENTS; ++componentId)
		{
			if (archetypeSignature.test(componentId))
			{
				...
				if (componentTypeInfo == nullptr)
				{
					...
					m_fatalErrorHandler.requestFatalTermination("Archetype Manager : ComponentTypeInfo not found for componentId in createNewArchetypeDefinition");
		...
		else
		{
			...
			m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Newly created ArchetypeDefinition not found in ArchetypeSignature-ArchetypeDefinition Map");
	...
	ArchetypeChunkHeader* ArchetypeManager::createArchetypeChunkHeader(ArchetypeDefinition* archetypeDefinition)
	{
		...
		if (archetypeChunkHeaderRawPtr == nullptr)
		{
				...
				m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Memory allocation failed for new ArchetypeChunkHeader");
			...
			else//2. The availableChunks for the Archetype is empty and need to create new chunks and put it in the availableChunks in ChunkList
			{
				...
				if (archetypeDef == nullptr)
				{
					assert(archetypeDef != nullptr);
						...
						m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to get archetypeDef , there is corruption somewhere or need to review logic in Chunklist and archetype lifetime relations ");
				...
				if (newArchetypeChunkHeader == nullptr)
				{
					assert(newArchetypeChunkHeader != nullptr);
						...
						m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to create new ArchetypeChunkHeader");
		...
		else//2. ChunkList of ArchetypeSiganture does not  exists and we may need to create the ChunkList and ArchetypeDefinition
		{
			...
			else//2. ArchetypeDefinition does not exsist so we need to create ArchetypeDefinition from ArchetypeSignature  , ChunkList for it , and create and put a ArchetypeChunkHeader into availableList in ChunkList
			{
				...
				if (newArchetypeDefinition == nullptr)
				{
					...
					m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to create new ArchetypeDefinition for signature");
	...
	ArchetypeManager::ArchetypeManager(IComponentRegistry& componentRegistry, IFatalErrorHandler& fatalErrorHandler) :
		...
		std::vector<EntityRecordUpdate> ArchetypeManager::addComponentToEntity(EntityAddInfo entityAddInfo)
		{
			...
			if (entityAddInfo.entityRecord.archetypeChunkHeader == nullptr) // 1. Initial add of data to archetype
			{
				...
				if (archetypeChunkHeader == nullptr)
				{
					assert(archetypeChunkHeader != nullptr);
						...
						m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to get ArchetypeChunkHeader for adding component to entity ");
			...
			else //2. Moving from old archetype to new archetype
			{
				...
				if (dstArchetypeChunk == nullptr)
				{
					...
					m_fatalErrorHandler.requestFatalTermination("Archetype Manager : Failed to get destination ArchetypeChunk for moving entity ");
